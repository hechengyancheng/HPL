...
### 事件系统实现
- **ActionContext** 中实现了完整的事件注册和触发机制
- 支持的事件类型：
  - `action` - 动作事件（如 player uses item）
  - `state` - 状态事件（如 health is 0）
  - `timer` - 计时器事件（如 bomb expires）
  - `event` - 随机事件
  - `game_start` - 游戏开始生命周期
  - `every_turn` - 每回合生命周期
- **Evaluator** 中实现了 `_register_event_handler` 方法
- 事件处理器支持条件检查和动作匹配
...
1. **编写标准库单元测试** - 为所有标准库动作编写测试
2. **编写游戏框架测试** - 测试事件系统、对话系统、房间切换
3. **编写完整游戏示例** - 创建一个可运行的文字冒险游戏
4. **修复导入问题** - 解决包导入链问题，使测试更顺畅
