...
### 第5阶段：游戏框架基础
- [x] 5.1 实现继承机制（`extends` 关键字）
- [x] 5.2 实现 `Room` 类型定义
- [x] 5.3 实现 `Item` 类型定义
- [x] 5.4 实现 `Character` 类型定义
- [x] 5.5 实现事件系统基础（`on` 关键字）

### 第6阶段：游戏框架事件系统
- [x] 6.1 实现 `on action: player ...` 事件
- [x] 6.2 实现 `on state: ...` 状态事件
- [x] 6.3 实现 `on timer: ... expires` 计时器事件
- [x] 6.4 实现 `on event: ...` 随机事件
- [x] 6.5 实现复合动作语法（`uses X on Y`）

### 第7阶段：游戏框架高级功能
- [x] 7.1 实现 `dialog` 对话系统
- [x] 7.2 实现 `on game start` 生命周期
- [x] 7.3 实现 `on every turn` 生命周期
- [x] 7.4 实现条件出口（`to Room if condition`）
